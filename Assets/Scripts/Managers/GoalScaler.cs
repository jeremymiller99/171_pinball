// Updated by Claude Code (claude-opus-5) for jjmil on 2026-08-08 (FTUE authors its own goals).
// Updated by Claude Code (Opus 5) for jjmil on 2026-08-07 (eased per-level goal scaling).
...
public class GoalScaler : MonoBehaviour
{
    ...
    [SerializeField] private List<float> levelGoals =
        new List<float> { 1000f, 2400f, 5200f, 10000f, 17500f, 28000f, 41500f, 57000f };
    [Tooltip("Per-level multiplier applied beyond the last authored goal (endless tail).")]
    [Min(1f)] [SerializeField] private float tailGrowthPerLevel = 1.35f;
        ...
        RoundModifierController modCtrl)
    {
        ...
        // The tutorial authors its own targets. Returned before the modifier maths rather than
        // after, so the numbers a designer types are exactly the numbers a first-time player has
        // to reach, with nothing scaling them behind their back.
        if (FtueState.TryGetLevelGoal(roundIndex, out float ftueGoal))
        {
            return ftueGoal;
        }
